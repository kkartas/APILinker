...
class FieldMapper:
    ...
    def _register_built_in_transformers(self) -> Dict[str, Callable]:
        ...
        return {
            ...
            "iso_to_timestamp": lambda v: int(datetime.fromisoformat(v.replace("Z", "+00:00")).timestamp()) if v else None,
            "timestamp_to_iso": lambda v: datetime.fromtimestamp(int(v)).isoformat() if v else None,
    ...
    def load_custom_transformer(self, module_path: str, function_name: str, alias: Optional[str] = None) -> None:
        """
        ...
        try:
            ...
            logger.info(f"Loaded custom transformer from {module_path}.{function_name} as {name}")
            
        except (ImportError, AttributeError) as e:
            logger.error(f"Error loading custom transformer: {str(e)}")
            raise ValueError(f"Failed to load transformer from {module_path}.{function_name}: {str(e)}")
    ...
    def add_mapping(self, source: str, target: str, fields: List[Dict[str, Any]]) -> None:
        ...
        for field in fields:
            if "source" not in field or "target" not in field:
                raise ValueError("Field mapping must contain 'source' and 'target' keys")
        ...
        logger.debug(f"Added mapping from {source} to {target} with {len(fields)} fields")
    ...
    def map_data(
        ...
        if not mapping:
            logger.warning(f"No mapping found for {source_endpoint} to {target_endpoint}")
            return data
    ...
    def _map_item(self, item: Dict[str, Any], fields: List[Dict[str, Any]]) -> Dict[str, Any]:
        """

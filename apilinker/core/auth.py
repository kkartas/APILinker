...
class AuthManager:
    ...
    def configure_auth(self, auth_config: Dict[str, Any]) -> AuthConfig:
        ...
        if auth_type == "api_key":
            ...
            required_params = ["client_id", "authorization_url", "token_url", "redirect_uri"]
            missing_params = [param for param in required_params if param not in auth_config]
            
            if missing_params:
                raise ValueError(f"OAuth2 PKCE authentication requires {', '.join(missing_params)} parameters")
            ...
            if missing_params:
                raise ValueError(f"OAuth2 Device Flow authentication requires {', '.join(missing_params)} parameters")
    ...
    def refresh_oauth2_token(self, auth_config: OAuth2ClientCredentials) -> OAuth2ClientCredentials:
        ...
        try:
            ...
            if self.secure_storage and hasattr(auth_config, "storage_key") and auth_config.storage_key:
    ...
    def complete_pkce_flow(self, auth_config: OAuth2PKCE, authorization_code: str) -> OAuth2PKCE:
        """
    ...
    def poll_device_flow(self, auth_config: OAuth2DeviceFlow) -> Tuple[bool, Optional[OAuth2DeviceFlow]]:
        """
        ...
        if not auth_config.device_code:
            raise ValueError("Device code is required to poll for device flow completion")

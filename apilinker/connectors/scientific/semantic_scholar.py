...
class SemanticScholarConnector(ApiConnector):
    ...
    def __init__(self, api_key: Optional[str] = None, **kwargs):
        ...
        # Set up headers
        headers = {"User-Agent": "ApiLinker/1.0.4"}
        if api_key:
    ...
    def search_papers(
        ...
        if fields is None:
            fields = [
                ...
                "venue", "publicationDate", "publicationTypes", "s2FieldsOfStudy"
            ]
    ...
    def get_paper_citations(self, paper_id: str, limit: int = 100) -> Dict[str, Any]:
        ...
        endpoint_path = self.endpoints["get_paper_citations"].path.format(paper_id=paper_id)
    ...
    def get_paper_references(self, paper_id: str, limit: int = 100) -> Dict[str, Any]:
        ...
        endpoint_path = self.endpoints["get_paper_references"].path.format(paper_id=paper_id)
    ...
    def get_author_papers(
        ...
        endpoint_path = self.endpoints["get_author_papers"].path.format(author_id=author_id)
    ...
    def _summarize_trends(self, trend_data: Dict[int, Dict]) -> Dict[str, Any]:
        ...
        paper_growth = (
            (trend_data[last_year]["total_papers"] - trend_data[first_year]["total_papers"]) /
            trend_data[first_year]["total_papers"] * 100
            if trend_data[first_year]["total_papers"] > 0 else 0
        )
        
        citation_growth = (
            (trend_data[last_year]["total_citations"] - trend_data[first_year]["total_citations"]) /
            trend_data[first_year]["total_citations"] * 100
            if trend_data[first_year]["total_citations"] > 0 else 0
        )
        
        return {
            "paper_count_growth_percent": round(paper_growth, 1),
            "citation_count_growth_percent": round(citation_growth, 1),
            "trend_direction": "growing" if paper_growth > 10 else "stable" if paper_growth > -10 else "declining",
            "research_maturity": "mature" if trend_data[last_year]["avg_citations_per_paper"] > 10 else "emerging"
        }
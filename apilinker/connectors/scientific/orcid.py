...
class ORCIDConnector(ApiConnector):
    ...
    def get_researcher_profile(
        ...
        if include_employment:
            try:
                ...
                profile["employment_summary"] = {"note": "Employment data not available"}
    ...
    def get_employment_history(self, orcid_id: str) -> Dict[str, Any]:
        ...
        employment_path = self.endpoints["get_employments"].path.format(orcid_id=clean_orcid)
    ...
    def get_education_history(self, orcid_id: str) -> Dict[str, Any]:
        ...
        education_path = self.endpoints["get_educations"].path.format(orcid_id=clean_orcid)
    ...
    def analyze_researcher_network(
        ...
        network_data["institutional_collaborations"] = self._analyze_institutional_collaborations(
            network_data["institutions"]
        )
    ...
    def _clean_orcid_id(self, orcid_id: str) -> str:
        ...
        clean_id = orcid_id.replace("https://orcid.org/", "").replace("http://orcid.org/", "")
        
        # Ensure proper format (0000-0000-0000-0000)
        if len(clean_id) == 16 and "-" not in clean_id:
            clean_id = f"{clean_id[:4]}-{clean_id[4:8]}-{clean_id[8:12]}-{clean_id[12:]}"
    ...
    def _extract_current_affiliation(self, profile: Dict[str, Any]) -> str:
        ...
        try:
            employments = profile.get("employment_summary", {}).get("employment-summary", [])
    ...
    def _extract_all_affiliations(self, profile: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            # From education
            educations = profile.get("education_summary", {}).get("education-summary", [])
    ...
    def _analyze_institutional_collaborations(self, institutions: Dict[str, List[str]]) -> List[Dict]:
        ...
        for i, inst1 in enumerate(institution_list):
            for inst2 in institution_list[i+1:]:
                shared_researchers = len(set(institutions[inst1]) & set(institutions[inst2]))
                ...
                if shared_researchers > 0:
                    collaborations.append({
                        ...
                        "collaboration_strength": "strong" if shared_researchers > 5 else "moderate" if shared_researchers > 2 else "weak"
                    })
        
        return sorted(collaborations, key=lambda x: x["shared_researchers"], reverse=True)
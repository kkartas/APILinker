...
class SentimentAnalysisTransformer(TransformerPlugin):
    ...
    NEGATIVE_WORDS = [
        ...
        "sad", "angry", "hate", "dislike", "broken", "error", "issue", "bug"
    ]
...
class WebhookConnector(ConnectorPlugin):
    ...
    def send(self, connection: Any, endpoint: str, data: Any, **kwargs) -> Dict[str, Any]:
        """
        Store webhook data.
...
class ApiKeyRotationAuth(AuthPlugin):
    ...
    def authenticate(self, **kwargs) -> Dict[str, Any]:
        ...
        current_key = self.primary_key if self.active_key == "primary" else self.secondary_key

...
def main():
    ...
    parser.add_argument("--dry-run", "-d", action="store_true", help="Dry run without creating issues")
    ...
    linker.mapper.register_transformer("github_labels_to_gitlab", github_labels_to_gitlab)
    ...
    # Print summary
    github_repo = linker.source.endpoints["list_issues"].path.split("/repos/")[1].split("/issues")[0]
    gitlab_project = linker.target.endpoints["create_issue"].path.split("/projects/")[1].split("/issues")[0]
    
    print(f"Migrating issues from GitHub repository: {github_repo} to GitLab project ID: {gitlab_project}")
